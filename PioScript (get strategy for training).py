...
directory = 'C:\\Users\\Gabriel\\Documents\\trees\\SRP\\PFC\\BU x CO\\Vs regs\\'
saveFolder = directory + 'Strategies in text\\Facing cbet 75\\'
...
def showStrategyPlusEv():

    print('Show strategy + ev')

    time.sleep(1)

    mouse.position = (1850, 320)
    mouse.click(Button.left)

    time.sleep(1)

def copyStrategyAndEV():

    print('Copy strategy and ev')
    ...
    mouse.position = (666, 666)
    mouse.click(Button.left)

    time.sleep(1)

def saveTextFile(flop):
    ...
    file = open(saveFolder + flop.split(':')[0] + '.txt', 'w+')
    file.write(data)
    file.close()
